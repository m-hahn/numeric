...
class Model():
   def __init__(self):
       self.conv1 = torch.nn.Conv2d(in_channels = 32, out_channels=128, kernel_size=3).cuda()
       self.conv2 = torch.nn.Conv2d(in_channels = 128, out_channels=128, kernel_size=3).cuda()
       self.emb = torch.nn.Embedding(10, 32).cuda()
       self.out = torch.nn.Linear(128, 1).cuda()
...
for iteration in range(100):
    ...
    pic[:,:,0] = 0
    pic[:,:,99] = 0
    print(pic[:, 0:99, 0:99].min())
    prediction, loss = model.forward(pic.long().cuda(), labels=pic[:, 0:99, 0:99].mean(dim=1).mean(dim=1).cuda()-1)
    loss = loss.mean()
    model.backward(loss)
    print(iteration, loss.cpu().sqrt())
